...
## 9. RELATED REPOSITORIES & PLUGIN INTEGRATION

### Overview of Related Repos

| Repo | What It Is | Relationship to 248 |
|------|-----------|---------------------|
| `wearehomeric-collab/claude-plugins-official` | Fork of Anthropic's official Claude Code plugin marketplace (28+ internal, 13+ external plugins) | Install plugins FROM this repo into Claude Code |
| `wearehomeric-collab/ralph` | Full Ralph autonomous agent toolkit (fork of snarktank/ralph) | `scripts/ralph/` is a partial copy; the full repo has PRD skills |
| `wearehomeric-collab/llm-council` | Multi-LLM orchestration (fork of karpathy/llm-council) — parallel query, peer review, synthesis | Adaptable for multi-model anthem lyric generation |

### Keep Repos Separate (Recommended)

**Do NOT move these repos into 248.** They serve different purposes:

- **`claude-plugins-official`** is a plugin marketplace — you install plugins from it, you don't embed it
- **`ralph`** is a general-purpose agent loop — it works across any project
- **`llm-council`** is a standalone app — adapt it separately for anthem-specific use

**248 should remain focused** on the anthem pipeline: Airtable schema, n8n workflows, API integrations, and project-specific code.

### How to Use Each Repo with 248

#### A. Claude Plugins (`claude-plugins-official`)

**Plugins to install for the anthem project:**

| Plugin | Why |
|--------|-----|
| `ralph-loop` | Official plugin version of Ralph — may be more maintained than the standalone repo |
| `commit-commands` | Better git commit workflow |
| `claude-code-setup` | Helps configure Claude Code for new projects |
| `claude-md-management` | Manages CLAUDE.md files across the project |
...
**External plugins worth exploring:**
- `slack` — if you want notifications when tracks are ready for QC
- `github` — enhanced GitHub integration

#### B. Ralph (`ralph`)

**What's missing from your local copy:**

Your `scripts/ralph/` has only `ralph.sh` + `CLAUDE.md`. The full repo adds:

| File | Purpose | Should You Add It? |
|------|---------|-------------------|
| `skills/prd/SKILL.md` | Generates structured PRDs from feature descriptions | Yes — useful for planning anthem features |
| `skills/ralph/SKILL.md` | Converts markdown PRD to `prd.json` format | Yes — needed if using Ralph for automated development |
| `prd.json.example` | Example showing expected JSON schema | Yes — reference for creating PRDs |
| `prompt.md` | Amp CLI support | Only if using Amp |
| `AGENTS.md` | Command reference for agents | Nice to have |
| `flowchart/` | React visualization app | No — not needed in 248 |

**Recommended action:** Either:
1. Install the `ralph-loop` plugin from `claude-plugins-official` (cleaner), OR
2. Copy the missing skills files into `scripts/ralph/skills/`

#### C. LLM Council (`llm-council`)

**How it could enhance anthem generation:**

The current anthem workflow uses a single AI per step (Perplexity for research, ChatGPT for lyrics). The LLM Council pattern could improve **Step 3 (Script Generation)** dramatically:

```
Current:  ChatGPT → single draft → human edits → final script

With Council:
  Stage 1: GPT-4o + Claude + Gemini each write lyrics independently
  Stage 2: All three review each other's lyrics anonymously, rank quality
  Stage 3: Chairman model synthesizes the best elements into final script
  Result:  Higher quality first drafts, less human editing needed
```

**To integrate:**
1. Keep `llm-council` as its own repo
2. Add an `OPENROUTER_API_KEY` to your 248 `.env`
3. Create an n8n workflow that calls the council API for script generation
4. Or create a `src/council-lyrics.ts` script in 248 that adapts the council pattern specifically for lyric generation

**Configuration change needed in llm-council:**
- Modify `backend/config.py` to use creative-writing-optimized models
- Change ranking criteria from "accuracy" to "creativity, rhythm, emotional impact"
- Add golf/anthem domain context to system prompts

### Plugin Architecture for 248

To make 248 itself plugin-compatible with Claude Code, create this structure:

```
248/
├── .claude-plugin/
│   └── plugin.json              # Register 248 as a Claude Code plugin
├── .claude/
│   └── mcp.json                 # MCP servers (n8n)
├── commands/
│   ├── new-anthem.md            # /new-anthem slash command
│   ├── research-course.md       # /research-course slash command
│   └── check-status.md          # /check-status slash command
├── skills/
│   └── anthem-workflow/
│       └── SKILL.md             # Auto-triggered anthem creation skill
└── ... (rest of project)
```

**Example: `/new-anthem` slash command** (`commands/new-anthem.md`):
```markdown
---
description: Create a new anthem request in Airtable
argument-hint: <course-name> <strategy-type>
allowed-tools: [Bash, Read, Write]
---

Create a new anthem request for the specified course and strategy type.
1. Look up the course in Airtable
2. Validate the strategy type (Smart/Aggressive/Conservative/Risk-Reward)
3. Create the ANTHEM_REQUEST record
4. Update status to "Pending Review"
5. Report the request ID back to the user
```
...
|----------|--------|--------|
| 1 (Now) | Install `ralph-loop` and `commit-commands` plugins | 5 min |
| 2 (Now) | Create `.claude/mcp.json` for n8n | 5 min |
| 3 (This week) | Copy Ralph skills into `scripts/ralph/skills/` | 15 min |
| 4 (This week) | Create anthem-specific slash commands in `commands/` | 1-2 hours |
| 5 (Next week) | Adapt LLM Council for multi-model lyric generation | 4-6 hours |
| 6 (Future) | Create full `anthem-workflow` skill | 2-3 hours |

---

*This plan was generated by analyzing all files in the 248 repository plus the three related repos (`claude-plugins-official`, `ralph`, `llm-council`). The project is currently documentation-only. The next step is to create the technical infrastructure (package.json, env config, Airtable client), configure the n8n MCP server, and install relevant plugins.*
