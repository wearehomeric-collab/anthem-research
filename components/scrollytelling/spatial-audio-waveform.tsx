...
// Audio annotations representing call-and-response patterns
...
// Generate waveform data with realistic-looking audio pattern
function generateWaveform(samples: number, seed: number = 0): number[] {
  ...
  for (let i = 0; i < samples; i++) {
    ...
    // Add envelope variations for more realistic audio look
...
export function SpatialAudioWaveform({ className, autoPlay = false }: SpatialAudioWaveformProps) {
  ...
  const cycleDuration = 8000 // 8 seconds for full visualization cycle
  ...
  // Update active annotations based on playhead position
  useEffect(() => {
  ...
  const getAnnotationColor = (type: Annotation["type"]) => {
    switch (type) {
      case "call": return "bg-orange text-black"
      case "response": return "bg-terra-cotta text-black"
      case "anchor": return "bg-cream-dark text-black"
      case "cue": return "bg-orange-dark text-cream-light"
      default: return "bg-charcoal-light text-cream-light"
    }
  }
          ...
          return (
            ...
            >
              ...
              <div
                ...
                style={{
                  backgroundColor: annotation.type === "call" ? "#ff6b35" : 
                                   annotation.type === "response" ? "#e07a5f" : 
                                   annotation.type === "anchor" ? "#e8dcc4" : "#d2691e",
                  borderRadius: "4px",
                }}
              />
  ...
  return (
    ...
    >
      ...
      <div className="mb-6 font-sans text-lg font-semibold text-cream-light">
      ...
      >
        {/* Left channel */}
        <div className="mb-4">
          {renderWaveform(leftWaveform, "#ff6b35", "left")}
        </div>
        ...
        {/* Right channel */}
        <div className="mt-4">
          {renderWaveform(rightWaveform, "#e07a5f", "right")}
        </div>
        ...
        <div
          ...
          style={{
            ...
            background: "linear-gradient(180deg, #ff6b35 0%, #e07a5f 100%)",
            boxShadow: "0 0 8px rgba(255, 107, 53, 0.6)",
          }}
        >
          {/* Playhead top indicator */}
          <div className="absolute -top-1 left-1/2 h-2 w-2 -translate-x-1/2 rotate-45 bg-orange" />
          {/* Playhead bottom indicator */}
          <div className="absolute -bottom-1 left-1/2 h-2 w-2 -translate-x-1/2 rotate-45 bg-terra-cotta" />
        </div>
