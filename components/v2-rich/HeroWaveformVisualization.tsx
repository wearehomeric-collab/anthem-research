...
//
// Rich animated 3:1 timing waveform with beat markers ported from
// wearehomeric-collab/anthem-research @ claude/anthem-orange-sBtvP
// at components/scrollytelling/hero-waveform-visualization.tsx.
//
// Canvas draws a complete measure (4 beats) as a waveform with a
// clear 3-beat backswing phase (cream/amber) and 1-beat downswing
// phase (orange). A playhead sweeps across at 112 BPM. Below the
// canvas: 4 beat markers that light up as the playhead passes them,
// with a thin progress bar underneath. Play/pause controls.
//
// This is the 12.9 kB rich version. The current components/
// HeroWaveformVisualization.tsx is a simpler 3.1 kB version used
// by V1 and left untouched.
//
// HOW TO MOUNT:
//   import { HeroWaveformVisualization } from "@/components/v2-rich/HeroWaveformVisualization"
//   <HeroWaveformVisualization className="..." />
//
// Uses the V2 dark palette orange (#F59E0B) via the alt-branch
// token names (bg-charcoal / text-cream-light / text-orange) that
// resolve in app/globals.css.
...
const COLORS = {
  ...
  background: "#0C1220",
  beatMarker: "#1E3A5F",
  beatMarkerActive: "#F59E0B",
  playhead: "#F59E0B",
  grid: "rgba(226, 232, 240, 0.1)",
  waveform: "rgba(226, 232, 240, 0.3)",
}

const BPM = 112
const BEATS_PER_MEASURE = 4
const MS_PER_BEAT = (60 / BPM) * 1000
const CYCLE_DURATION = MS_PER_BEAT * BEATS_PER_MEASURE
...
export function HeroWaveformVisualization({ className }: { className?: string }) {
  ...
  const generateWaveformPoints = useCallback(
    (width: number, height: number): WaveformPoint[] => {
      ...
      for (let i = 0; i <= numPoints; i++) {
        ...
        if (phase === "backswing") {
          ...
          y =
            centerY +
            Math.sin(localProgress * Math.PI * frequency) * amplitude * envelopeRise * 0.8
        } else {
          ...
          y =
            centerY +
            Math.sin(localProgress * Math.PI * frequency + Math.PI) *
              amplitude *
              envelopeFall *
              1.2
        }
  ...
  const beatPhases: ("backswing" | "downswing")[] = [
    "backswing",
    "backswing",
    "backswing",
    "downswing",
